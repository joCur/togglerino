...
func (h *ProjectMemberHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	member, err := h.members.UpdateWithAdminCheck(r.Context(), project.ID, userID, model.ProjectRole(req.Role))
	if err != nil {
...
func (h *ProjectMemberHandler) Remove(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.members.RemoveWithAdminCheck(r.Context(), project.ID, userID); err != nil {
		if errors.Is(err, store.ErrLastAdmin) {
