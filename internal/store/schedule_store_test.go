...
func setupScheduleTest(t *testing.T) (*store.ScheduleStore, *store.FlagStore, *model.Flag, *model.Environment, string) {
	...
	flag, err := fs.Create(ctx, project.ID, uniqueKey("schflag"), "Schedule Flag", "test", model.ValueTypeBoolean, model.FlagTypeRelease, json.RawMessage(`false`), []string{}, nil, nil, nil)
	if err != nil {
